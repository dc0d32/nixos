...
Item {
  ...
  Process {
    ...
    command: ["sh", "-c",
      ...
      "printf '%s\\n%s\\n' \"$(cat $b/capacity 2>/dev/null)\" \"$(cat $b/status 2>/dev/null)\"; exit 0; done"]
    running: true
    stdout: StdioCollector { onStreamFinished: {
      const lines = text.trim().split("\n")
      if (lines.length >= 2 && lines[0] !== "") {
        root.present = true; root.percent = parseInt(lines[0]) || 0; root.status = lines[1]
  ...
  RowLayout {
    ...
    Text { font.family: Theme.font; font.pixelSize: 12; color: Theme.subtext; text: root.percent + "%" }
  }
