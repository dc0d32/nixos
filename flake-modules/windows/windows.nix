...
let
  ...
  # ── Alacritty on Windows: DISABLED ──────────────────────────────────
  # Alacritty was dropped as a Windows terminal (2026-08-31). On Windows it
  # delegates glyph fallback to DirectWrite's system fallback, which — unlike
  # Linux fontconfig — cannot be pointed at a monospace face, so glyphs
  # missing from the primary font fall back to a *proportional* face and
  # overlap adjacent cells, with no config-level fix (alacritty/crossfont
  # use `FontFallback::get_system_fallback()`; alacritty#4899 is maintainer
  # wontfix: "font fallback is done through the system"). Windows Terminal,
  # which DOES support an explicit font-fallback list (`face = "A, B"`), is
  # the Windows terminal now.
  #
  # NOTHING below is deleted — the full Alacritty wiring is kept but gated on
  # this one flag so re-enabling is a single flip: the scoop package, config
  # generation (`alacrittyToml`), the deploy step, and the RDP Mesa
  # software-GL step (setup.ps1 Step 6) all key off `enableAlacritty`. While
  # it is false, setup.ps1 additionally `scoop uninstall`s any previously
  # installed copy (see `scoopRetired`), so flipping it off actively removes
  # Alacritty on the next `hm_win`/`--setup` run rather than merely ceasing
  # to install it.
  ...
  wingetCli = [
    ...
    # btop4win — the native-Windows port of btop. winget ships it as a
    # portable zip (installs under %LOCALAPPDATA%\Microsoft\WinGet\
    # Packages\…, user-writable). Its config (btop.conf) is generated below
    # from the shared flake.lib.btopSettings and deployed next to the exe
    # by hm_win — btop4win reads config only from its own folder.
  ...
  # GPU terminal — DISABLED (see `enableAlacritty` above); kept, gated, so
  # re-enabling is one flag flip. On Scoop (not winget) deliberately: the
  # RDP Mesa software-GL step (setup.ps1 Step 6) drops an app-local
  # opengl32.dll next to alacritty.exe, and Scoop's per-user
  # apps\alacritty\current\ is writable without admin, unlike winget's
  # Program Files install. `extras` bucket (added by setup.ps1). While the
  # flag is false this is omitted from installs and setup.ps1 uninstalls
  # any old copy (see `scoopRetired`).
  ++ lib.optional enableAlacritty "alacritty";

  # The patched Fantasque terminal face is generated by Nix and deployed by
  # hm_win below; no Scoop font package is needed.
  scoopFonts = [ ];

  # Scoop apps to actively uninstall on the next setup.ps1 run (retired or
  # feature-disabled). alacritty lands here whenever `enableAlacritty` is
  # false, so turning the flag off removes the installed copy rather than
  # just no longer installing it. Empty list ⇒ the setup.ps1 loop is a
  # no-op (`foreach ($app in @())`).
  scoopRetired =
    lib.optional (!enableAlacritty) "alacritty"
  ...
  wingetRetired = [
    ...
    "astral-sh.ruff"
    "chmln.sd"
    # Alacritty is retired on Windows (enableAlacritty = false — see the
    # flag's comment above). It was previously moved winget -> Scoop for
    # the RDP Mesa GL step; it now lives on neither. Kept in this
    # uninstall list so setup.ps1 removes any lingering winget copy on
    # machines that still have it. Also uninstalled from Scoop via
    # scoopRetired.
    "Alacritty.Alacritty"
  ];
  ...
  mkWindowsDotfiles = pkgs:
    let
      ...
      windowsTerminalSettings =
        ...
        (pkgs.formats.json { }).generate "windows-terminal.json" {
          ...
          profileDefaults = {
            ...
            cursorShape = "bar";
            font = {
              # The selected Fantasque face includes grid-safe terminal
              # symbols, so DirectWrite never reaches an oversized fallback
              # for btop triangles, Braille spinners, or clock faces.
              face = "FantasqueSansM Terminal Nerd Font Mono";
      ...
      psmuxConf = pkgs.writeText "psmux.conf" ''
        ...
        #
        # Mouse is deliberately OFF on Windows (it is `on` in the Linux
        # tmux.nix — this is a Windows-only override). psmux 3.3.8 forwards
        # mouse reports to a pane as literal keystrokes when it decides the
        # pane's program does not want the mouse (see psmux configuration.md
        # "@mouse-force"). Under ConPTY, conhost echoes a TUI's
        # `ESC[?1003;1006h` all-motion mouse-enable into the output stream,
        # and psmux's per-window mouse-ownership state desyncs across window
        # switches: after btop (or any mouse TUI) has run, the outer terminal
        # is left stuck in all-motion (1003) tracking, so every idle mouse
        # move floods the focused pane as `<35;col;rowM` garbage — corrupting
        # btop on return-to-window and spraying the shell prompt. Turning
        # psmux mouse off stops psmux enabling/forwarding mouse entirely, which
        # eliminates the garbage. Trade: no psmux wheel-scroll or click/drag on
        # Windows — scrollback is copy-mode (prefix `[`) + PgUp / vi keys.
        # Retire when psmux fixes window-switch mouse-mode reassertion under
        # ConPTY (tracking upstream: issues #597/#604 mouse keepalive/reassert).
        set -g mouse off
        setw -g mode-keys vi
        set -g base-index 1
      ...
      # btop4win config — mirrors the Linux btop (flake.lib.btopSettings)
      # so both monitors look the same, with a few Windows-specific tweaks.
      # btop4win reads btop.conf ONLY from the same folder as btop4win.exe
      # (GetModuleFileNameW; no ~/.config or env-var override), so hm_win
      # resolves the installed exe's directory at deploy time and drops this
      # file beside it. btop4win rewrites btop.conf with every default on
      # exit, so these values only need to seed it; hm_win re-seeds on run.
      #
      # Divergences from the shared settings, all Windows-only (these must
      # NOT leak into the Linux/mac btop, which is why they live here as
      # overrides rather than in the shared flake.lib.btopSettings):
      #   - cpu_graph_lower: the shared value "iowait" doesn't exist on
      #     Windows. btop4win's own default here is "gpu", but the winget
      #     package is the non-LHM build with no GPU sensor (that's a
      ...
      btopConf =
        ...
        pkgs.writeText "btop.conf" (
          "#? Generated by the nixos flake (hm_win) — mirrors flake-modules/btop.nix\n"
          + lib.concatStringsSep "\n" (lib.mapAttrsToList render winSettings)
          + "\n"
        );
      ...
      setupScript = pkgs.writeText "setup.ps1" ''
        ...
        # Remove Scoop copies of retired / feature-disabled apps (e.g.
        # alacritty when enableAlacritty is false), so turning a feature off
        # actively uninstalls it on the next run rather than leaving it.
        foreach ($app in @(${lib.concatMapStringsSep ", " (a: "'${a}'") scoopRetired})) {
          if (Test-ScoopApp $app) {
            Write-Host "  scoop uninstall $app (retired/disabled)"
            scoop uninstall $app
          }
        }
        ...
        '' else ''
        Write-Host '== Step 6/6: (alacritty disabled) skipping Mesa software GL =='
        ''}
    ...
    pkgs.runCommand "windows-dotfiles" { } ''
      ...
      ${lib.optionalString enableAlacritty ''cp ${alacrittyToml}        "$out/alacritty.toml"''}
      ...
      cp -r ${pkgs.fantasque-terminal}/share/windows-fonts "$out/fonts"
  ...
  mkHmWin = pkgs: pkgs.writeShellApplication {
    ...
    text = ''
      ...
      ${lib.optionalString enableAlacritty ''
      ...
      if [ "$run_setup" -eq 1 ]; then
        ...
        # setup.ps1 just installed btop4win via winget; the earlier deploy
        # pass ran before that and skipped btop.conf. Land it now.
        deploy_btop_conf
      fi

      # Install the Nix-generated Fantasque face after setup.ps1 has tried to
      # remove the old Scoop-owned copy. This uses a distinct internal family
      # name and content-addressed filenames, so an open terminal cannot lock
      # either the initial replacement or a later update.
      font_dir="$userprofile/AppData/Local/Microsoft/Windows/Fonts"
      mkdir -p "$font_dir"
      installed_fonts=()
      for font in "$src"/fonts/*.ttf; do
        stem="$(basename "$font" .ttf)"
        digest="$(sha256sum "$font" | cut -c1-12)"
        installed="$font_dir/$stem-$digest.ttf"
        if [ ! -e "$installed" ]; then
          cp "$font" "$installed"
        fi
      ...
      if [ -n "$psexe" ]; then
        font_dir_win="$(wslpath -w "$font_dir")"
        font_dir_b64="$(printf '%s' "$font_dir_win" | base64 | tr -d '\n')"
        "$psexe" -NoProfile -NonInteractive -Command \
          "& {
            \$FontDir = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('$font_dir_b64'))
            \$key = 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Fonts'
            New-Item -Path \$key -Force | Out-Null
            \$acl = Get-Acl -LiteralPath \$FontDir
            ...
            Set-Acl -LiteralPath \$FontDir -AclObject \$acl
          }"
        for installed in "''${installed_fonts[@]}"; do
          installed_win="$(wslpath -w "$installed")"
          stem="$(basename "$installed" .ttf)"
          registry_name="''${stem%-????????????} (TrueType)"
          installed_b64="$(printf '%s' "$installed_win" | base64 | tr -d '\n')"
          registry_name_b64="$(printf '%s' "$registry_name" | base64 | tr -d '\n')"
          "$psexe" -NoProfile -NonInteractive -Command \
            "& {
              \$Path = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('$installed_b64'))
              \$Name = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('$registry_name_b64'))
              New-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows NT\CurrentVersion\Fonts' -Name \$Name -Value \$Path -PropertyType String -Force | Out-Null
              Add-Type -Name FontInstaller -Namespace HmWin -MemberDefinition '
                [System.Runtime.InteropServices.DllImport(\"gdi32.dll\", CharSet = System.Runtime.InteropServices.CharSet.Unicode)]
                public static extern int AddFontResourceEx(string name, uint flags, System.IntPtr reserved);
                [System.Runtime.InteropServices.DllImport(\"gdi32.dll\", CharSet = System.Runtime.InteropServices.CharSet.Unicode)]
                public static extern bool RemoveFontResourceEx(string name, uint flags, System.IntPtr reserved);
              ...
              '
              while ([HmWin.FontInstaller]::RemoveFontResourceEx(\$Path, 0, [IntPtr]::Zero)) {}
              [void][HmWin.FontInstaller]::AddFontResourceEx(\$Path, 0, [IntPtr]::Zero)
              [void][HmWin.FontInstaller]::PostMessage([IntPtr]0xffff, 0x001d, [IntPtr]::Zero, [IntPtr]::Zero)
            }"
        done
        for stale in "$font_dir"/FantasqueSansMTerminalNerdFontMono-*.ttf; do
          keep=false
          for installed in "''${installed_fonts[@]}"; do
            if [ "$stale" = "$installed" ]; then keep=true; break; fi
          done
          if [ "$keep" = false ]; then
            stale_win="$(wslpath -w "$stale")"
            stale_b64="$(printf '%s' "$stale_win" | base64 | tr -d '\n')"
            "$psexe" -NoProfile -NonInteractive -Command \
              "& {
                \$Path = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('$stale_b64'))
                Add-Type -Name FontRemover -Namespace HmWin -MemberDefinition '
                  [System.Runtime.InteropServices.DllImport(\"gdi32.dll\", CharSet = System.Runtime.InteropServices.CharSet.Unicode)]
                  public static extern bool RemoveFontResourceEx(string name, uint flags, System.IntPtr reserved);
                  [System.Runtime.InteropServices.DllImport(\"user32.dll\")]
                  public static extern bool PostMessage(System.IntPtr window, uint message, System.IntPtr wParam, System.IntPtr lParam);
                '
                while ([HmWin.FontRemover]::RemoveFontResourceEx(\$Path, 0, [IntPtr]::Zero)) {}
                [void][HmWin.FontRemover]::PostMessage([IntPtr]0xffff, 0x001d, [IntPtr]::Zero, [IntPtr]::Zero)
              }"
            rm -f "$stale" 2>/dev/null || true
          fi
        done
        echo "  Fantasque terminal font installed"
      else
        echo "  WARN: Windows PowerShell unavailable; font files copied but not registered"
      fi
