...
let
  ...
  mkWindowsDotfiles = pkgs:
    let
      ...
      # works on native Windows. psmux is tmux-compatible and reads
      # ~/.psmux.conf. Deliberately dropped from the Linux config because
      # they need Unix terminfo / are latency workarounds / use format
      # conditionals psmux doesn't implement:
      #   - `terminal tmux-256color` + `terminal-overrides …:Tc` (no
      #     terminfo on Windows; ConPTY negotiates truecolor itself)
      ...
      # Kept: mouse, vi copy-mode, 1-based indexing, big scrollback,
      # window renumbering, and the |/- split binds — all plain settings.
      psmuxConf = pkgs.writeText "psmux.conf" ''
        # Generated by the nixos flake (hm_win). Mirrors the portable
        # subset of flake-modules/tmux.nix; edit there, not here.
        set -g mouse on
        setw -g mode-keys vi
        set -g base-index 1
