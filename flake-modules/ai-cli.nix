# AI assistant CLIs.
#
# These are user-scoped tools (they read user config from
# $XDG_CONFIG_HOME and auth tokens from the user's keyring), so they
# belong in home-manager rather than environment.systemPackages.
#
# - github-copilot-cli: `ghcs`/`ghce` shell suggestions, requires
#   `gh auth login` (handled by ./gh.nix) and `gh extension install
#   github/gh-copilot` on first use.
# - opencode: this very tool. Config lives at ~/.config/opencode/.
#
# Migrated from modules/home/tools/ai-cli.nix.
{
  flake.modules.homeManager.ai-cli = { pkgs, ... }: {
    home.packages = with pkgs; [
      github-copilot-cli
      opencode
    ];
  };
}
