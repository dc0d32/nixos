  ...
  config = {
    ...
    flake.modules.nixos.biometrics = { lib, pkgs, ... }: {
      ...
      environment.systemPackages = [
        ...
        #
        # The script body lives at ../scripts/biometrics-enroll.sh
        # (real .sh file, not inline) so editor tooling, shellcheck
        # in CI, and direct execution during local development all
        # work without Nix antiquoting hazards. writeShellApplication
        # additionally runs shellcheck at build time.
        ...
        (pkgs.writeShellApplication {
          ...
          # set doesn't include it. (The script also sets these
          # itself for direct-execution safety, but the duplication
          # is harmless.)
