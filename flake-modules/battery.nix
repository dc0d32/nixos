...
# at the flake-parts level would make it a global singleton and any
# second laptop host would conflict on `resumeDevice` (which IS
# per-host data). Hosts set `battery = { … };` inside their
# `configurations.nixos.<name>.module` block.
#
